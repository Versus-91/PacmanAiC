...
class PacmanAgent:
    def __init__(self):
        ...
        self.target = DQN(input_shape=451, num_actions=4).to(device)
        self.policy = DQN(input_shape=451, num_actions=4).to(device)
        # self.load_model()
    ...
    def calculate_reward(
        self, done, lives, hit_ghost, action, prev_score, info
    ):
        reward = 0
        if done:
            if lives > 0:
